...
struct process *create_process(const void *image, size_t image_size) {
    ...
    *--sp = (uint32_t) user_entry;
    ...
    for (paddr_t paddr = __kernel_base;
...
void kernel_main(void) {
    ...
    // __asm__ __volatile__("unimp"); // unip == csrrw x0, cycle, x0 --> causes error since 'cycle' is read-only 
    ...
    read_write_disk(buf, 0, false); //false --> 'read from the disk'
    printf("first sector: %s\n", buf);

    strcpy(buf, "hello from kernel!!!\n");
    read_write_disk(buf, 0, true); //true --> 'write to the disk' */
